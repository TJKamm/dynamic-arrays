...
//colorList.pop(); //this removes the last/end element (and //returns the element)
//assign the end element to a result variable
//var removedElement = colorList.pop();

//colorList.shift(); //removes the first element in the array

//colorList.unshift("Brown"); //adds an element to the front/at //the start of the array

//colorList.splice(0,2); //splice takes out 1 or many elements
//first argument is the START position
//second element is HOW MANY to take out
...
//creat a new array (list) to hold the purchased //images
var purchasedImages = [];
...
{
  //we only want to src attribute of the image so we need to get the "src" attribute of the img tag we click on
  var onlyPath = evt.target.getAttribute("src");
  
  //console.log(evt.target); //bring back just the element that //is clicked
  //evt.target.setAttribute("src", "images/drawing/thumbs/artwork_22.jpg");
  //evt.target.setAttribute("class", "blackborder");
